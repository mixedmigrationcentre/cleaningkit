...
#' @param macro Logical. If \code{TRUE}, a macro-enabled \code{.xlsm} is written instead of a
#'   \code{.xlsx}, carrying a VBA change-capture macro: every edit a reviewer makes on the
#'   \code{dataset} sheet is appended automatically to the bottom of the cleaning log, so nothing
#'   has to be copied across by hand. See \strong{Macro-enabled logs} below. Requires
#'   \code{include_dataset = TRUE} and a non-\code{NULL} \code{output_path}. Default \code{FALSE},
#'   so existing behaviour is untouched.
#' @param macro_issue_prefix Prefix used to build the \strong{Issue} value of macro-appended rows.
#'   The Nth edit of a given cell is written as \code{"<prefix>_00N"}. Default \code{"manual_edit"}.
#' @param vba_project Path to the compiled \code{vbaProject.bin} to inject. Default \code{NULL}
#'   uses the one shipped with the package (see \code{ck_vba_project_path()}). Only used when
#'   \code{macro = TRUE}.
#'
#' @section Macro-enabled logs:
#' With \code{macro = TRUE} the workbook gains a VBA project and two very-hidden sheets:
#' \code{_ck_config} (the sheet and column names the macro reads, so nothing is hard-coded) and
#' \code{_ck_ledger} (the original value, latest value and edit count of every cell a reviewer has
#' touched). Editing a cell on the \code{dataset} sheet appends a row to the cleaning log with
#' \strong{Old value}, \strong{New value}, \strong{Question number}, \strong{Question text},
#' \strong{Survey UUID}, \strong{Date}, \strong{Enumerator} and a mapped \strong{Action taken}
#' (\code{addition} when the cell was empty, \code{delete_data_point} when it is cleared,
#' \code{recoded} otherwise).
#'
#' \strong{Repeat edits.} Editing the same cell again appends \emph{another} row rather than
#' revising the first, so the log keeps the full history. Each row's \strong{Old value} is the
#' value immediately before that edit, so successive rows chain raw -> A -> B. To keep those rows
#' distinct through \code{read_cleaning_log()}, whose deduplication key is
...
#' \code{apply_cleaning_log()} applies the most recent value last.
#'
#' \strong{Formatting.} Appended rows have their formatting cleared before they are written, and
#' \code{Application.ExtendList} is suspended during the write, so none of the
#' \code{create_formated_wb()} row colours carry down onto them. They are given the same
#' \strong{Action taken} drop-down as the rest of the log.
#'
#' \strong{Prerequisite.} The compiled VBA project is not generated by R - it is built once from
#' the sources in \code{inst/vba/} using \code{dev/build_vba_bin.R} (or by hand, see
#' \code{inst/vba/README.md}) and shipped as \code{inst/extdata/cleaningkit_vba.bin}. Until it
#' exists, \code{macro = TRUE} raises an error explaining how to build it.
#'
#' \strong{Reading the result back.} \code{.xlsm} files are read by \code{readxl} exactly like
#' \code{.xlsx}, but remember to widen \code{file_pattern} when scanning a directory with
#' \code{read_cleaning_log()}.
#'
#' @return A workbook object, or (when \code{output_path} is given) writes a \code{.xlsx} file -
#'   or a \code{.xlsm} when \code{macro = TRUE} - invisibly.
#' @export
#'
#' @examples
#' \dontrun{
...
#' )
#'
#' # macro-enabled log: edits on the dataset sheet append themselves to the log
#' create_cleaning_log(
#'   write_list,
#'   macro = TRUE,
...
create_cleaning_log <- function(
  ...
  # ---- macro pre-flight ----
  # Fail here rather than after building the whole workbook: the checks are
  # cheap and the build is not.
  macro <- isTRUE(macro)
  if (macro) {
    if (!isTRUE(include_dataset)) {
      stop(
        "`macro = TRUE` needs the raw data in the workbook for the reviewer to edit. ",
        "Set `include_dataset = TRUE`."
      )
    }
    if (is.null(output_path)) {
      stop(
        "`macro = TRUE` writes a macro-enabled file, so it needs an `output_path` ",
        "(ending in .xlsm). A VBA project cannot be attached to a workbook object."
      )
    }
    if ("_ck_config" %in% names(write_list)) {
      stop(
        "The list already has an element named `_ck_config`. Please rename it."
      )
    }
    resolved_vba <- if (is.null(vba_project)) ck_vba_project_path() else vba_project
    if (!nzchar(resolved_vba) || !file.exists(resolved_vba)) {
      stop(
        "No VBA project found, so `macro = TRUE` cannot be honoured.\n",
        "The compiled `vbaProject.bin` is built once from the sources in `inst/vba/` ",
        "- R cannot generate it. Run `dev/build_vba_bin.R` on a Windows machine with ",
        "Excel, or follow `inst/vba/README.md`, then try again.\n",
        "Use `macro = FALSE` in the meantime to write a plain .xlsx."
      )
    }
  }

  # Row layout of the dataset sheet as it will be written: openxlsx puts the
  # column names on row 1, so the ONA label row lands on row 2 and the records
  # start on row 3. Without a label row everything shifts up by one.
  macro_label_row <- if (isTRUE(skip_label_row)) 2L else 0L
  macro_first_data_row <- if (isTRUE(skip_label_row)) 3L else 2L
  ...
  # The macro reads every name it needs from this sheet, so a single compiled
  # vbaProject.bin serves every log the package produces. It goes last so the
  # positions of the existing sheets are unchanged.
  if (macro) {
    out_list[["_ck_config"]] <- ck_macro_config_df(
      ...
      header_row = 1L,
      label_row = macro_label_row,
      first_data_row = macro_first_data_row
    )
  }
  ...
  # the macro's configuration is machinery, not content: keep it out of the
  # sheet tabs and out of the unhide dialog
  if (macro) {
    config_sheet <- which(names(workbook) == "_ck_config")
    if (length(config_sheet) == 1) {
      openxlsx::sheetVisibility(workbook)[config_sheet] <- "veryHidden"
    }
  }
  ...
  # openxlsx has no supported way to attach a VBA project, so the workbook is
  # written normally and the OPC package is patched afterwards. See
  # add_vba_project() for what that involves and why.
  if (!grepl("\\.xlsm$", output_path, ignore.case = TRUE)) {
    ...
    message(
      "create_cleaning_log: `macro = TRUE` writes a macro-enabled workbook; ",
