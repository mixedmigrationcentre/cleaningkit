#' Path to the VBA project shipped with cleaningkit
#'
#' Returns the path to \code{inst/extdata/cleaningkit_vba.bin}, the compiled VBA
#' project injected into macro-enabled cleaning logs. Returns \code{""} when the
#' binary has not been built yet - see \code{inst/vba/README.md} and
#' \code{dev/build_vba_bin.R}.
#'
#' @return A file path, or \code{""} if the VBA project is not present.
#' @export
#'
#' @examples
#' \dontrun{
...
ck_vba_project_path <- function() {
  path <- system.file("extdata", "cleaningkit_vba.bin", package = "cleaningkit")
  if (!nzchar(path) || !file.exists(path)) "" else path
}
...
add_vba_project <- function(
  ...
  if (!nzchar(vba_project) || !file.exists(vba_project)) {
    stop(
      ...
      "The macro-enabled cleaning log needs a compiled `vbaProject.bin`, which ",
      "is built once from the sources in `inst/vba/` and is not generated by R.\n",
      "Run `dev/build_vba_bin.R` on a Windows machine with Excel, or follow the ",
      "manual steps in `inst/vba/README.md`, then re-run with `macro = TRUE`.\n",
      "Until then, use `macro = FALSE` to write a plain .xlsx."
    )
  }
